...
async function run() {
  ...
  try {
    ...
    try {
      ...
      if (
        serialized.includes('No active ProBro connection') ||
        serialized.includes('Configured remote ProBro agent') ||
        serialized.includes('No automatic fallback was attempted')
      ) {
      ...
      if (
        msg.includes('No active ProBro connection') ||
        msg.includes('Configured remote ProBro agent') ||
        msg.includes('No automatic fallback was attempted')
      ) {
        gotExpectedFailure = true;
      }
    }
